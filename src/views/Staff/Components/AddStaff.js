...
const AddStaff = ({mode, open, handleClose, initial_data}) => {
  ...
  const [title, setTitle]  = useState('Add Staff');
    const [formState, setFormState] = useState({ 
     values: {  id: '', firstname: '', lastname: '', staffId: '',  email: '', branch_id: '', phone: '',  address: ''},
      touched: {},
        errors: {}
      });
  useEffect(() => {
    ...
    return () => {
      setFormState(prev => ({
        ...prev,
      values: { 
        ...prev.values, id: '', firstname: '', lastname: '', staffId: '', branch_id: '',  email: '', phone: '',  address: ''}
      }))
    }
  }, [initial_data, mode]);
  useEffect(() => {
    ...
    setFormState(formState => ({
      ...formState,
      isValid: errors.firstname.error || errors.lastname.error || errors.staffId.error || errors.email.error || errors.branch_id.error ?  false: true,
      errors: errors || {}
    }));
  }, [formState.values]);
...
const handleSubmit = e => {
  ...
  const resetForm = () => {
    setFormState(prev => ({
      ...prev,
      values: { ...prev.values,  id: '', firstname: '', lastname: '', staffId: '', branch_id: '',  email: '', phone: '',  address: ''},
      touched: {
        ...formState.touched,
        firstname: false,
        lastname: false,
          ...
          <Row>
              ...
              <Col md="12"> 
                <FormGroup>
                    ...
                    <Input
                    type="text" 
                    value={formState.values.staffId || ''}
                    name="staffId"
                    id="staffId"
                    onChange={handleChange} 
                    placeholder="Staff ID"
                    invalid={ hasError('staffId')}
                    />
                  <FormFeedback>
                    {  
                    hasError('staffId') ? formState.errors.staffId && formState.errors.staffId.message : null
                      } 
